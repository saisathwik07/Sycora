...
organizationSchema.pre('findOneAndDelete', async function(next) {
  ...
  if (organizationId) {
    ...
    await mongoose.model('User').updateMany(
      ...
      { $pull: { memberships: { organization: organizationId } } }
    );
    await mongoose.model('User').updateMany({ organization: organizationId }, { $unset: { organization: 1 } });
    await mongoose.model('JoinRequest').deleteMany({ organization: organizationId });
    await mongoose.model('OrgInvitation').deleteMany({ organization: organizationId });
    await mongoose.model('Notification').deleteMany({ 'payload.organizationId': organizationId });
  }
  next();
});
